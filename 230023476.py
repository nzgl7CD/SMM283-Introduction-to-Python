...
class PortfolioVisualising:
    ...
    def _create_widgets_interface(self):
        ...
        self.correlation_matrix_button = ttk.Button(self.frame, text='Edit Correlation Matrix', command=self.open_correlation_popup)
    ...
    def open_correlation_popup(self):
        ...
        for r in range(rows):
            ...
            for c in range(cols):
                if r == c:
                    lbl = tk.Label(popup, text=self.current_matrix[r][c], width=8, justify=tk.CENTER, bg='#D3D3D3', font=('Inter', 10))
                    lbl.grid(row=r, column=c, padx=5, pady=5)
                    row_entries.append(None)  # No Entry widget here
                else:
                    e = tk.Entry(popup, width=8, validate='key', validatecommand=float_validator, justify=tk.CENTER, font=('Inter', 10), bg='#FFFFFF', fg='black', highlightbackground='#000000', highlightcolor='#0000FF', highlightthickness=1)
                    e.insert('end', self.current_matrix[r][c])
                    e.grid(row=r, column=c, padx=5, pady=5)
                    row_entries.append(e)
            self.entries.append(row_entries)

        save_button = ttk.Button(popup, text='Save', command=lambda: self.save_correlation_matrix(popup))
    ...
    def update_fields(self, portfolio_size):
        ...
        self.correlation_matrix_label.config(text=self.correlation_matrix_var.get())  # Update label content
...
def main():
    ...
    exit_button = ttk.Button(app.frame, text="Exit", command=lambda:exit(), style='TButton')
